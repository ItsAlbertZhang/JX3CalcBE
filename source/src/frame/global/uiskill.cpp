...
void UISkillManager::add(int skillID, int skillLevel) {
  ...
  gdi::Interface::tabSelect(gdi::Tab::skillrepresent, arg);
  if (arg.size() == 0) {
    /* Fuzzy select */
    arg.emplace_back();
    arg[0]["SkillID"] = std::to_string(skillID);
  ...
  if (arg.size() == 0) {
    ...
    sr.tab["Name"] = ns_frame::SkillManager::get(skillID, skillLevel).SkillName;
    sr.Name = sr.tab["Name"];
    data[std::make_tuple(skillID, skillLevel)] = std::move(sr);
    return;
  }
  sr.tab = std::move(arg[0]);
