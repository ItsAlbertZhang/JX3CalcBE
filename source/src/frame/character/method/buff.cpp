...
BuffItem *Character::buffGet(int buffID, int buffLevel) {
    ...
    if (0 == buffLevel) {
        ret = buffGetWithCompareFlag(buffID, buffLevel + 1, static_cast<int>(Ref<ref::lua::BUFF_COMPARE_FLAG>::Type::GREATER_EQUAL));
    } else {
        ret = buffGetWithCompareFlag(buffID, buffLevel, static_cast<int>(Ref<ref::lua::BUFF_COMPARE_FLAG>::Type::EQUAL));
    }
    if (ret != nullptr) {
...
BuffItem *Character::buffGetByOwner(int buffID, int buffLevel, int sourceID) {
    BuffItem *ret = nullptr;
    if (0 == buffLevel) {
        ret = buffGetByOwnerWithCompareFlag(buffID, buffLevel + 1, sourceID, static_cast<int>(Ref<ref::lua::BUFF_COMPARE_FLAG>::Type::GREATER_EQUAL));
    } else {
        ret = buffGetByOwnerWithCompareFlag(buffID, buffLevel, sourceID, static_cast<int>(Ref<ref::lua::BUFF_COMPARE_FLAG>::Type::EQUAL));
    }
    if (ret != nullptr) {
        this->buffFlushLeftFrame(ret);
    }
