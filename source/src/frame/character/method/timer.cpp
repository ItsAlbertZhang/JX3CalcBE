...
static void callbackSetTimer(void *self, void *param) {
    ...
    if (!LuaCpp::analysis(LuaCpp::getOnTimer(data.data.idx)(selfPtr, data.data.type, data.data.targetID), data.data.idx, LuaCpp::Func::OnTimer))
...
void Character::timerSet(int frame, std::string filename, int targetID) {
    ...
    Data data{
        .data = {static_cast<uint16_t>(LuaCpp::getIndex(filename)), static_cast<uint16_t>(type), static_cast<uint32_t>(targetID)}
    };
