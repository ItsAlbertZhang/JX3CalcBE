...
int main(int argc, char *argv[]) {
  ...
  // 初始化接口
  ret = gdi::Interface::initGameData(ns_program::Config::pJX3,
                                     ns_program::Config::pUnpack);
  std::cout << "initGameData = " << ret << std::endl;
  if (!ret)
    return 0;

  ret = gdi::Interface::initLua(ns_framestatic::luaInit,
                                ns_framestatic::luaFuncStaticToDynamic);
  ...
  for (auto &it : player.chDamage.damageList) {
    const ns_frame::Skill &skill =
        ns_frame::SkillManager::get(it.skillID, it.skillLevel);
    std::cout << std::fixed << std::setprecision(2) << it.tick / 1024.0 << "s\t"
              << it.skillID << "\t" << it.skillLevel << "\t" << it.isCritical
              << "\t" << it.damage << "\t" << static_cast<int>(it.damageType)
              << "\t" << skill.tab.at("SkillName") << std::endl;
  }
...
//     // 在 lua 中定义一个函数, 该函数接受一个 Character 类的实例, 修改其 id,
//     并调用 CastSkill lua.script(R"(
//         function f(c)
//             c.id = 123
//             c:CastSkill(456)
//         end
